...
const EditFestival = ({ festivali,organizatori, firebaseUrl,adminMode }) => {
  ...
  const [sveSlike, setSveSlike] = useState(festival.slike)

  // Fetch the existing festival data and populate the form fields
  useEffect(() => {
    const fetchFestivalData = async () => {
      try {
        const response = await fetch(`${firebaseUrl}/festivali/${id1}/${id2}.json`);
        if (!response.ok) throw new Error('Failed to fetch festival data');
        const festivalData = await response.json();
        setFormData(festivalData);
        setSveSlike(festival.slike || [])
      } catch (error) {
